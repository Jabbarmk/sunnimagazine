...
export async function GET() {
  const [rows] = await db.query("SELECT id, email, role FROM admins ORDER BY id ASC");
...
export async function POST(req: Request) {
  ...
  if (b.id) {
    if (b.password?.trim()) {
      ...
      await db.query("UPDATE admins SET email=?, role=?, password=? WHERE id=?", [email, b.role || "admin", hash, b.id]);
    } else {
      await db.query("UPDATE admins SET email=?, role=? WHERE id=?", [email, b.role || "admin", b.id]);
    }
  ...
  const [result] = await db.query(
    ...
    [email, hash, b.role === "super_admin" ? "super_admin" : "admin"]
  );
