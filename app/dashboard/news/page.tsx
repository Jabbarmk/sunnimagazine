...
const EMPTY = { categoryId: "", title: "", description: "", image: "", source: "", publishedAt: "" };
...
export default function NewsPage() {
  ...
  const reload = () => { getNews().then(setItems); getNewsCategories().then(setCats); };
  useEffect(reload, []);
  ...
  const handleAddCat = async () => {
    ...
    await saveNewsCategory({ id: "ncat_" + Date.now(), name: newCatName.trim() });
  ...
  return (
    <div className="max-w-2xl space-y-6">
      <div>
        ...
        <p className="text-[13px] text-gray-500 mt-1">{items.length} items</p>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-5">
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <input value={form.title} onChange={set("title")} placeholder="News title"
              className={inp("title")} />
          ...
          <div>
            ...
            <input value={form.publishedAt} onChange={set("publishedAt")} placeholder="e.g. May 7, 2026"
        ...
        {items.map((item) => (
          <div key={item.id} className="bg-white rounded-xl border border-gray-200 flex items-start gap-3 p-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              {item.publishedAt && <div className="text-[11px] text-gray-400">{item.publishedAt}</div>}
            </div>
